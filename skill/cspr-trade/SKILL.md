---
name: cspr-trade
description: Guide users through CSPR.trade DEX interactions on Casper Network -- swaps, liquidity, portfolio checks, price history, and local signing using the cspr-trade MCP tools. Use when a user asks about trading tokens on Casper, checking prices or candles, swapping CSPR/USDT, adding or removing liquidity, checking LP positions or impermanent loss, viewing portfolio value, or inspecting swap history on CSPR.trade.
---
